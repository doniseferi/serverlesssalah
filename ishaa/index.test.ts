...
describe('ishaa function', () => {
  test.each([
    ...
    test('Returns a date time in iso 8601 format string in the body', async () => {
      ...
      const response: SalahResponse = await ishaa(context, request)
      const data = response.body as Salah
      expect(data).not.toBe(null)
      expect(data.salah).toBe('ishaa')
      expect(data.utc).toBe('2037-08-02T19:45:25.263Z')
    }),
    test('Returns a valid error response on consecutive error calls', async () => {
      ...
      expect((await ishaa(context, request)).body as SalahError).not.toBe(null)
      expect((await ishaa(context, request)).body as SalahError).toBe(
        (await ishaa(context, request)).body as SalahError,
      )
    }),
    test.each([
